...
void main(){
  ...
  //   color = mix(color, vec3(uv.y, 1., 1.),stroke(polySDF(uv, 30.), .5, .5));
    ...
    iri *= sin(length(color.rg) * 20.0 + t);

    color = mix(color,iri,sin(t - sin(uv.x/uv.y)));
    // pMod2(color.rg, vec2(uv.x, uv.y));
    // pMod2(uv, vec2(length(uv.x), uv.y));

    // color += vec3(stroke(cnoise( rote *4. * cnoise(roteC * 4.) ), uv.x, .2));

    color = mix(color, vec3(uv.y,uv.x,1.), stroke(cnoise( rote *4. * cnoise(roteC * 4.) ), uv.y, .2));
    color = vec3(uv.x, uv1.y, iri.x);
    color = mix(color, vec3(uv.x), stroke(snoise(uv1 * 2. * cnoise(rote1 * 10.) * roteC1.y), .2, .4));

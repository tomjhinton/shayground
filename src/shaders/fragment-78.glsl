...
vec2 bulge = vec2(0.);
float d = 0.;
int circlesArrLength = 8;
vec3 circle = vec3 (.5,.5, .8  );

uv.x += stroke(cnoise(rote * 1. * uv.x), .5,.5);
...
vec3 pat2 = vec3(1.-stroke(noisePix(rote * 9. * snoise(rote) ), .2, .8));
...
// color = mix(iri, color, cnoise(uv1 * 30.));
// spin(color.rb, .5);
// pMod2(color.rg, uv);
// uvRippleStatic(uv1, .3);
color += stroke(cnoise( vec2(uv / (sin(vTime * .5) +1.8) /2.) * 4. * noisePix(rote * 3. * uv.y)), .2 ,.5);
uvRipple(color.rg, .9);

color *= stroke(polySDF(uv1, 16. ), .4, .9);


if(color == vec3(0.)){
  color += stroke(cnoise( vec2(pattern2 * (sin(vTime * .5) +1.1) /2.) * 4. * noisePix(roteC * 30. * uv.y)), .2 ,.5);
  uvRipple(color.br, .9);

...
color.r +=  abs(cos(uv.x / 2.0) + sin(uv.y / 2.0) - cos(vTime));
color.g +=  abs(cos(uv.x / 4.0) - sin(uv.y / 4.0) + cos(vTime));
color.b +=  abs(sin(uv.x / 2.0) - sin(uv.y / 2.0) + sin(vTime));

vec3 square = shape(rote, 4);
color*= square;
vec3 square2 = fract(shape(roteC, 9));
color.b+= square2.g;
color.r+= uv.x;
color.g+= uv.y;

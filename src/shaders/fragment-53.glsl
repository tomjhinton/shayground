...
float smoothMod(float x, float y, float e){
    ...
    float bot = pow(sin(PI * (x/y)),2.)+ pow(e, sin(vTime));
...
void main(){
  ...
  // vec2 uv = vUv;
  vec3 brightness = vec3(0.6, .43, .9);
  vec3 contrast = vec3(.2, .2, .5);
  vec3 osc = vec3(4., 1., 0.5);
  vec3 phase = vec3(length(uv) *10., 122.02, 6.);
  ...
  float xSmoothMod = smoothMod(uv.x, .3, .2);
  float ySmoothMod = smoothMod(uv.y, .3, .2);

  float smoothMix = (sin(t - atan(uv.y/uv.x * tan(t +uv.y))* 2.) + 1.0)/2.0;

    uv.x = mix(uv.x, xSmoothMod, smoothMix);
    uv.y = mix(uv.y, xSmoothMod, 1.-smoothMix);

    uv.x = uv.y + sin(uv.x * 5.) * 1. * t;
    uv.y = clamp(abs(uv.y), .65, .5);



  // uv = uv*  1.0 + abs(sin(t));
  color = cosPalette((fract(uv.x/uv.y + uv.x) * .25 + cos(t * 1.5)), brightness, contrast, osc, phase );

  vec3 iri = hsv2rgb(vec3(color.x, color.y, color.y *.6));

  vec3 mixed = mix(color, iri, sin(t + uv.x/uv.y));

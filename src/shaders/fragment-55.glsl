...
void main(){
  ...
  vec3 color = cosPalette( snoise(rote * 2. * cnoise(roteC * 20.) * uv.y));
  ...
  //
    color *= mix(color, vec3(0., uv.y, uv.x),stroke(polySDF(uv2, 3.), .5, .5));

    color *= mix(color, vec3(uv.y, 1., 1.),stroke(polySDF(uv2, 30.), .5, .5));

    brownConradyDistortion(color.bg, -1., 1. * sin(vTime));
    spin(color.rb, .5);

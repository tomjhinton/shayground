...
void main(){
  ...
  color += mix(color, 1.-color, stroke(polySDF(roteC, 4. ), .2, 4.3));
  ...
  color = mix(color, 1.-color, stroke(raysSDF(roteC, 10. ), .2 , .8));
  color = mix(color, 1.-color, stroke(raysSDF(rote, 3. ), .2 , .8));

  color *= snoise(uv * 400.);
  uvRipple(color.rb, .9);
  color += snoise(color.rg * 4.);
  // uvRipple(color.rb, .09);
  // uvRipple(color.gr, .03);
  // uvRipple(color.br, .01);
  // uvRipple(color.gb, .02);
  // uvRipple(color.bg, .07);

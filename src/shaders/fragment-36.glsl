...
void main(){
  ...
  // uv.x = stroke(snoise(uv * 6. ), .2, .5);
  // uv.y = stroke(snoise(uv * 60. ), .2, .5);
  uvRipple(uv, sin(uv.x));

  color.r += stroke(snoise(uv * 4.), .2, .5) - uv.x;
  color.b += stroke(snoise(uv * 5.), .2, .5) + uv.y;
  color.g += stroke(snoise(uv * 6.), .2, .5);

  uvRipple(color.rg, 1.9);
  uvRipple(color.rb, 1.9);
  uvRipple(color.br, .9);
  uvRipple(color.bg, 1.7 * sin(vTime));
  uvRipple(color.gr, 1.7 * sin(vTime));
  color.r += sin(vTime);



  color = bridge(1.2-color, stroke(snoise(rote * 5. ), 1.2, 1.5), 1.5, .8);
  color += stroke(triangleDF(uv2), .8, .5);


  // color *= fill(triangleDF(uv1), 1.18);
  // color -= fill(rectSDF(uv1, vec2(.6 , .2 + wiggly(uv.x + vTime * .05, uv.y + (vTime * .5) * .005, 2., .6, .05))), 1.2);
 //   color+=uv.y * .5;
 // if(uv.x > uv.y){
 //   color+=uv.x * .5;
 // }
 float sphere = sphereSDF(vec3(uv1.xy, uv.x/uv.y));
 color = vec3(sphere);
 color = mix(color, vec3(uv.x, uv1.y, 1.), .5);
 color = mix(color * .5,1.-color, uv.y);
 pMod2(color.rb, vec2(.5, .2));
 spin(color.rb, .5);
  // color -= fill(triangleDF(rote), 1.18);
  // color += fill(triangleDF(roteC), .88);
  // color -= fill(triangleDF(rote), .78);
  // color += fill(triangleDF(rote), .68);
  color = mix(color ,1.-color, uv2.y);
  color = mix(color ,1.-color, snoise(uv2 * 2.));
  pMod2(color.br, vec2(.5 ));
  pMod2(color.gr, vec2(.2));
  pMod2(color.rg, vec2(.4));
  color += .4;
  color = mix(color, .5-color, stroke(snoise(uv1 *2.), .2, .2));

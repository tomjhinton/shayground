...
vec2 bulge = vec2(0.);
float d = 0.;
int circlesArrLength = 18 ;
vec3 circle = vec3 (.5,.5, .8  );


// uv.x+= stroke(cnoise(rote * 10. * uv.x), .5,.5);
// uv.y += stroke(cnoise(roteC *3. * uv.y), .5, .6);
// uvRipple(uv,.9);

float growth = 0.01 +circle.z * 8.;
float r = growth * .022;

bulge += (uv ) * scale/float(circlesArrLength)  * pow((length(stroke(cnoise(uv *3. * uv.y), .5, .6)) ), .5  );
d += smoothstep(r, r*0.5 , length(uv -circle.xy )-r);
// d+= stroke(cnoise(uv * 5.), .5, .5);
...
float pattern2 = mod(id.x+pattern , 3.);
float pattern3 = mod(pattern + pattern2 , 4.);
vec3 color = vec3(pattern, pattern2, pattern3);


//

vec3 pat1 = vec3(stroke(noisePix(uv * 19. * snoise(uv1) ), .2, .4), stroke(noisePix(rote * 9. * snoise(uv1) ), .2, .2), stroke(cnoise(rote * 9. * snoise(uv1) ), .2, .8) );
vec3 pat2 = vec3(stroke(noisePix(uv * 9. * snoise(uv) ), .2, .8));


float red  = add(pattern, pat1.x);
float green  = add(pattern2, pat2.y);
float blue  = add(pattern3, pat2.x);
color = vec3(red,green,blue);
color = mix(color, pat2, pattern);
...
//
vec3 iri = hsb2rgb(vec3(color.x, color.z, uv.y *.3));
// pMod2(color.zy, vec2(.5));
iri += vec3(tan(length(iri.xz) * 8.0 + t));
//
color = mix(color, 1.-color, pattern3);
color = mix(vec3(pattern, iri.x, iri.y), color, cnoise(uv * 3.));
// spin(color.rb, .5);
// pMod2(color.rg, uv);
// uvRippleStatic(uv1, .3);
color += stroke(cnoise( vec2(uv / (sin(vTime * .5) +1.8) /2.) * 4. * noisePix(rote * 30. * uv.y)), .2 ,.5);
color = mix(color, 1.-color, uv.x * PHI);
// uvRipple(color.rg, .9);
float meh = add(stroke(polySDF(uv1, 3. ), .4, .5), stroke(noisePix(roteC * 3. ), .4, .5));
color *= meh;



// if(color == vec3(0.)){
//   color.g += stroke(cnoise( vec2(pattern2 + (sin(vTime * .5) +1.1) /2.) * 4. * noisePix(roteC * 30. * uv.y)), .2 ,.5);
//
//     color.b += stroke(cnoise( vec2(pattern * (sin(vTime * .5) +1.1) /2.) * 60. * noisePix(roteC * 200. * uv.y)), .2 ,.5);
//
//     color.r += stroke(cnoise( vec2(pattern3 * (sin(vTime * .5) +1.1) /2.) * 9. * noisePix(roteC * 10. * uv.y)), .2 ,.5);
//
//
// }

...
void main(){
  ...
  vec3 color = vec3(0.);
  // pMod2(rote, vec2(.5));
  color.r = stroke(snoise(uv * 2. ),
  .2 +wiggly(uv.x + vTime * .005, uv.y + vTime * .005, 2., 6. , 1.5  ), .5);

  color.g = stroke(snoise(uv * 1. ),
  .2 +wiggly(uv.x + vTime * .005, uv.y + vTime * .005, 2., 6. , 1.5  ) , .2);

  color.b = stroke(snoise(rote * 1.  ),
  .2 +wiggly(uv.x + vTime * .005, uv.y + vTime * .005, 2., 6. , 1.5  ) , .8 );



  float squareO = rectSDF(roteC, vec2(1.,.5));
  float square = rectSDF(rote, vec2(1.,.5));
  color = mix(color, 1.-color, stroke(squareO, .8+wiggly(uv.x + vTime * .005, uv.y + vTime * .05, 2., 6. , 0.05  ), .15 )) ;
  // square +=color.r;
  // color = bridge(color, stroke(square, .8+wiggly(uv.x + vTime * .005, uv.y + vTime * .05, 2., 6. , 0.05  ), .15 ), .5,.5);

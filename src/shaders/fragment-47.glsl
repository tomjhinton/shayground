...
void main(){
  ...
  color = vec3(0., 0., 1.);
  // uvRippleStatic(color.rg, .9);

  // spin(color.rg, .5);
  // spin(color.gb, .3);

  uv = brownConradyDistortion(rote, 1.2, -3.2);
  color.rb = brownConradyDistortion(roteC, 0.2, -3.2);
  color.br = brownConradyDistortion(roteC, -0.2, -4.2);
  color += snoise(color.rg * 4. * cnoise(uv * 40.) + rote );
  color += snoise(color.rb * 4. * color.r  +cnoise(uv * 40.));
  uvRipple(color.gb, .9);
  color.b *= stroke(cnoise(uv * 40. * cnoise(roteC * 40. * cnoise(rote * 4. * cnoise(uv * 4. * cnoise(roteC * 4. * cnoise(rote * 4. ) ) ) ) ) ),.6,.6);
  //
  // color.r *= stroke(cnoise(uv1 * 40. * cnoise(roteC * 4. * cnoise(rote * 4. * cnoise(uv * 4. * cnoise(roteC * 4. * cnoise(rote * 4. ) ) ) ) ) ),.6,.6);

  vec2 offset = vec2(.2,0.);

  color *= stroke(polySDF(uv, 3.), .5, .4);
  color += stroke(polySDF(uv1, 3. + sin(vTime * .5)), .3, .4);

  color = mix(color, 1.-color, cnoise(uv1 * uv.x * cnoise(rote * 40.)));
  // color *= color.r;
  // color += stroke(polySDF(uv + offset, 3.), .3, .2);

  // color.b += noise(rote * 100. * uv.x); // Black splatter
  // color.r -= noise(uv * 100.); // Holes on splatter
  // color+= .3;

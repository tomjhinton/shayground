...
void main(){
  ...
  uv = vec2(uv.x+ noisePix(rote * noisePix(uv * sin(vTime))), (uv.x * .5+ snoise(roteC * 1.5)));
  ...
  float smoothMix = (tan(t - atan(uv.x/uv.y*tan(t+uv.y))*2.)+1.0)/2.0;
    ...
    vec3 color = cosPalette( sin(vTime));

  color.g += mix(uv.x, xSmoothMod, smoothMix);
  color.b += mix(uv.y, ySmoothMod, 1.-smoothMix);
  ...
  //
    uv2 =  brownConradyDistortion(uv2, -1., -1. + sin(vTime * .5));
    color = mix(color, vec3(uv.x, uv.y, 1.),stroke(polySDF(uv2, 20.), .6, .5));
  // //
  ...
  //   //
    color = mix(color, vec3(uv1.x,uv1.y,1.), stroke(cnoise( uv1 *4. * noisePix(uv1 * 4. + wiggly(uv.x + vTime * .05, uv.y + (vTime * .5) * .005, 2., .6, .5)) ), .5, .8));
  //   //

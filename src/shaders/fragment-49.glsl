...
void main(){
  ...
  uv = brownConradyDistortion(uv, -1., 1. + sin(vTime) );
  color.b += brownConradyDistortion(roteC, 0.2* sin(vTime), -3.2 * sin(vTime)).y;
  color.b += brownConradyDistortion(rote, -0.2, -4.2 ).x;

  color.r += brownConradyDistortion(roteC, -0.4* sin(vTime), -2.2 * sin(vTime)).y;
  color.r += brownConradyDistortion(rote, -0.4, -5.2 ).x;

  color.g += brownConradyDistortion(roteC, 10.8* sin(vTime), -1.2 * sin(vTime)).y;
  color.g += brownConradyDistortion(rote, -0.8, -6.2 ).x;
  // vec2 brS = rotateUV(uv, vec2(.5), -PI * vTime * .05 );
  // color.rg = brS;

  uvRipple(color.rb, brownConradyDistortion(uv, -1., 1. + sin(vTime)).x);

  // color.b+= uv.y;

  color *= stroke(snoise(roteC * 2. * cnoise(rote *40.) * uv.y), .2, .2);
  // color.r -= wiggly(uv.x + vTime * .05, uv.y + (vTime * .5) * .005, 2., .6, 1.5);

  color.g += brownConradyDistortion(roteC, 10.8* sin(vTime), -1.2 * sin(vTime)).y;
  color += stroke(snoise(roteC * 2. * cnoise(rote *40.) * uv.y), .2, .2);


  color = mix(color, 1.-color, brownConradyDistortion(roteC, 3.8* sin(vTime * .5), -1.2 * sin(vTime * .3)).y +stroke(snoise(uv * 2. * cnoise(rote *40.) * uv.y), .2, .2));

  uv2 = brownConradyDistortion(uv2, -0.8, -2.2 );

  vec2 rote9 = rotateUV(uv2, vec2(.5), PI * 2. *( vTime * .05));
  vec3 color2 =color *stroke(polySDF(rote9, 50.), .6, .4);


  color *= stroke(polySDF(uv2, 2.), .2, .2);
  color = mix(color, color2, 1.5);


  if(color == vec3(0.)){
  color += stroke(snoise(uv * 4. * snoise(uv2 * 40.)), .5, .2);
  }

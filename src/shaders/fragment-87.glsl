...
void main() {
  ...
  float elevation2 = getElevation(vec2(pc), 2. );
  vec3 color = vec3(pc.y, pc.x, elevation2);
  color = mix(color, vec3(elevation), cheapNoise(color, uv.x * 20.));
  color.g = mix(color, 1.-color, cnoise(pc * elevation)).g;
  float tri = spiralSDF(pc, .3);
  float tri2 = triangleDF(rote);


  color *= stroke(tri, .6, .375 );
  color = mix(color, 1.-color, spiralSDF(uv + .5, .5));


// alpha = color.g + .2;

  color *= vec3(stroke(cnoise(rote *  40. * cnoise(roteC *  4.)), .5, cnoise(rote *  40. * cnoise(roteC *  4.))));

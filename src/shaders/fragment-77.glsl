...
bulge += (uv ) * scale/float(circlesArrLength)  * pow((length(uv) ), .5 +  wiggly(roteC.x + vTime * .05, roteC.y + vTime * .005, 2., 6. + sin(vTime * .05), 1.5  ) );
...
color *= stroke(triangleDF(uv1 ), .7, .5);


if(color == vec3(0.)){
  color += stroke(cnoise( rote * 4. * noisePix(roteC * 30. * uv.y)), .5 ,.5);
  // uvRipple(color.br, .9);
  spin(color.rb, .5);
  pMod2(color.rg, uv);
}

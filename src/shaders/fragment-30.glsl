...
void uvRipple(inout vec2 uv, float intensity){
  // float intensity=0.09;
	...
	vec2 p =-1.+2.*gl_FragCoord.xy / uResolution.xy-vec2(0,-uMouse.y*.001);


    float cLength=length(p);

     uv=(gl_FragCoord.xy - uResolution * .5) / uResolution.yy + 0.5
        +(p/cLength)*cos(cLength*15.0-vTime*4.0)*intensity;
...
void main(){
  ...
  color.g += shape(roteS, 6).r;

  vec3 color2 = vec3(0.);


  color *= stroke(triangleDF(uv2), .75 + wiggly(uv.x + vTime * .05, uv.y + (vTime * .5) * .005, 2., .6, .05), .2);
  color += fill(triangleDF(uv2), .9);
  color -= fill(triangleDF(uv2), .8);
  color += fill(triangleDF(uv2), .7);
  color -= fill(triangleDF(uv2), .6);
  color += fill(triangleDF(uv2), .5 * sin(vTime));
  pMod2(rote, vec2(.3, .5));
...
if( color == vec3(1.)){
  color += snoise(rote * 3.);
  color.r += snoise(rote * 30.);
  color.g += snoise(rote * 20.);
  color.b += snoise(rote * 10.);
}

vec2 roteUv2 = rotateUV(uv2, vec2(.5), PI * vTime * .05);


  float spi = stroke(spiralSDF(rote, 10.16 ), .3, .8  );

  color = mix(color, 1.-color, stroke(snoise(roteUv2 * 2.5), .2 + wiggly(roteUv2.x + vTime * .5, roteUv2.y + (vTime * .5) * .005, 1., .6, .5), .2));

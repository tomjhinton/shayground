...
float t = vTime ;
float scale =rote.x*roteC.y + 8.;
vec2 mouse = uMouse;
vec2 bulge = vec2(0.);
float d = rote.x/rote.y;
int circlesArrLength = 18 ;
vec3 circle = vec3 (.5,.5, .8  );


// uv.x+= stroke(cnoise(rote * 10. * uv.x), .5,1.5);
...
vec3 pat2 = vec3(stroke(noisePix(rote * 9. * snoise(rote) ), .2, .8));


float red  = add(pattern, pat1.x);
float green  = add(pattern2, pat2.y);
float blue  = add(pattern3, pat1.y);
color = vec3(red,green,blue);
color = mix(color, pat2, pattern);
color = mix(color, vec3(cnoise(roteC * 14.)), pattern2);
color = mix(1.-color, vec3(cnoise(uv * 30. * uv.y), cnoise(color.rg * 7. * uv.y), cnoise(uv * 130. * uv.y)) , pattern3);
//
...
color = mix(vec3(pattern, iri.x, iri.y), color, cnoise(uv * 30.));
spin(color.rb, .5);
pMod2(color.rg, uv);
uvRippleStatic(uv1, .3);
color *= stroke(cnoise( vec2(uv / (sin(vTime * .5) +1.8) /2.) * 4. * noisePix(rote * 30. * uv.y)), .2 ,.5);
color = mix(color, 1.-color, pattern);
// uvRipple(color.rg, .9);
// float meh = add(stroke(polySDF(rote, 5. ), .4, 1.9), stroke(polySDF(roteC, 3. ), .4, 1.9));
// color *= meh;

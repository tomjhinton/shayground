...
void main(){
  ...
  pMod2(uv, brownConradyDistortion(uv, 1., -sin(uv.x)+ cos(vTime * .5)));
  ...
  uv = brownConradyDistortion(uv, 1., -sin(uv.x)+ cos(vTime * .5));
...
//
  ...
  color.b += stroke(polySDF(vec2(uv.x, uv3.y * sin(vTime)), uv.y), uv.y, uv3.x);
  //
  vec4 final = mix( vec4(color.br, iri.g, 1.), vec4(iri.rg, color.r, 1.), stroke(noisePix(uv * 200. * uv.y), .6, .5));

...
float cheapNoise(vec3 stp, float mixVal) {
   ...
   vec3 getTerrainColor(float elevation){
      ...
      vec3 forestColor = vec3(10., .4, 0.1);
...
void main() {
  ...
  uvRipple(radialUv, 0.1);
  // uvX(radialUv, sin(vTime));
  radialUv.y -= sin(vTime * .005) * .001;
  radialUv.x -= vTime * .002;



  float elevation = getElevation(radialUv, stroke(cnoise(rote * 40. * sin(vTime * .05) ), .5, .5 ) );
  ...
  spin(color.rg, .5);
  // alpha =color.r+;
  color = mix(color, 1.-color,  stroke(cnoise(pc * 4. * sin(vTime * .05) ), .5, .5 ));

  color = mix(color, 1.-color,  stroke(cnoise(vec2(elevation, color.r) * 9. * cos(vTime * .05) ), .5, sin(vTime) +.1 ));

  color *= vec3(stroke(cnoise(vec2(elevation, radialUv.y) * 9. * cos(vTime * .05) +.1 ), .05, .6 ));

    alpha = vec3(stroke(cnoise(vec2(elevation, radialUv.y) * 90. * cos(vTime * .05) +.1 ), .05, .6 )).r;

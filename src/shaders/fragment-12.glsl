...
void main(){
  ...
  vec3 color = vec3(.0);
    ...
    color += bridge(vec3(cnoise(rote * 4.0* sin(slowTime)   )), color.g, .8, .075);
    ...
    color *= stroke(tri, .4* tan(slowTime), .25 * ((sin(vTime)+1.)/2.));
    color /= stroke(tri2, .45* tan(slowTime), .25 * atan(vTime));

...
void main(){
  ...
  vec2 rote2 = rotateUV(uv2, vec2(.5), PI * vTime * .04);
  vec2 roteC2 = rotateUV(uv2, vec2(.5), -PI * vTime * .04 );

  vec2 rote3 = rotateUV(uv3, vec2(.5), PI * vTime * .03);
  vec2 roteC3 = rotateUV(uv3, vec2(.5), -PI * vTime * .03 );
  ...
  color.r += shape(rote, 4).r;
  color.b += shape(roteC, 5).r;
  color.g += shape(rote, 6).r;


  color.r += shape(rote2, 4).r;
  color.b *= shape(roteC2, 5).r;
  color.g += shape(rote2, 6).r;

  color.r += shape(rote3, 4).r;
  color.b += shape(roteC3, 5).r;
  color.g += shape(rote3, 6).r;


  color = mix(color, 1.-color, uv.x);
  color = mix(color, 1.-color, uv.y);
  vec2 roteC4 = rotateUV(uv1, vec2(.5), -PI * vTime * .03 );

  // sin(x*.1)*20.
  // pMod2(uv, vec2(.2, .5));
  color = mix(color, 1.-color, stroke(snoise(uv4 * 100.),.2 ,uv4.y*8. - uv4.y ));

  vec3 color2 = vec3(0.);
  color2 -= fill(triangleDF(uv4), .9);
  color2 += fill(triangleDF(uv4), .8);
  color2 -= fill(triangleDF(uv4), .7);
  color2 += fill(triangleDF(uv4), .6);

  color2 -= fill(triangleDF(uv4), .5);
  color2 += fill(triangleDF(uv4), .4);
  color2 -= fill(triangleDF(uv4), .3);
  color2 += fill(triangleDF(uv4), .2);
  color2 -= fill(triangleDF(uv4), .1 );

 color2.r += stroke(snoise(uv4 * 200.), .2, .1 );
  color2.g += stroke(snoise(uv4 * 100.), .2, .1 );
  color2.b += stroke(snoise(uv4 * 50.), .2, .1 );


  color = mix(color, color2, uv.y);

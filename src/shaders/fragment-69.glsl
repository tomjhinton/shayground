...
void main(){
  ...
  uv.y += stroke(cnoise(roteC * uv.y * 12.), .4, .6);




  uv = brownConradyDistortion(uv, 10., -sin(uv.x)+ cos(vTime * .05));

  color.r = stroke(uv.x, sin(rote.x), uv.y);
...
//
  ...
  color.r *= stroke(polySDF(vec2(uv3.x, uv.y), 3.), 1.5, uv3.x);
  color.b += stroke(polySDF(vec2(uv.x, uv3.y * sin(vTime * .05)), uv.y), uv.y, .5);
  //
  color += stroke(polySDF(uv3, 4.), .5, .3);
  vec4 final = mix( vec4(color.gr, iri.g, 1.), vec4(iri.rg, color.b, 1.), stroke(noisePix(uv * 2. * uv.y), .1, .1));

  final.b *= length(.5);

  final.g /= stroke(noisePix(uv3 * 8. * uv3.y), .2, .5);

  final = mix(final, vec4(iri.rg, final.b, 1.), triangleDF(uv4));

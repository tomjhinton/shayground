...
float cheapNoise(vec3 stp, float mixVal) {
   ...
   float getElevation(vec2 uv){
     ...
     elevation += cnoise(uv * 80.) * .2;
     elevation += cnoise(uv * 200.) * .1;
     elevation += cnoise(uv * 900.) * .15;
     return elevation;
   }
   ...
   vec3 getTerrainColor(float elevation){
      ...
      //set colors
      vec3 sandColor = vec3(.96, .64, .38);
      vec3 forestColor = vec3(0., .4, 0.1);
   ...
   float getShadow(float elevation, vec2 uv){
     ...
     float towardElevation = getElevation( uv + towardSun * .001);


     float iterations = 5.;
     float shadow = 0.;
     for(float i = 1.0; i<=iterations; i++){
       float towardElevation = getElevation(uv + towardSun * .01 * i);
...
void main() {
  ...
  color -= getShadow(elevation, radialUv);

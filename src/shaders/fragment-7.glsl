...
vec2 pos = vUv - 0.5;
vec2 size = vec2(sin(vTime* 0.5), cos(vTime* 0.5));
...
pModPolar(pos, cos(vTime * 0.2)*18.0);
float shape2 = sdEquilateralTriangle(pos);


pos += vec2(sin(vTime) * 0.2, cos(vTime * 0.2) * 0.2);
pModPolar(pos, cos(vTime * 0.2)*3.0);
float shape3 = sdBox(pos, vec2(0.09));
shape3 = ceil(shape3);

vec3 col = cosPalette((vTime* 0.2) + min(shape, shape3), brightness, contrast, osc, phase);
vec3 col2 = cosPalette((vTime* 0.3) + min(shape2, shape3), brightness, contrast2, osc, phase);
col += col2;
